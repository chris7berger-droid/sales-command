...
Last updated: 2026-05-08 (tiers introduced; B6 closed retro; S2 added)
...
| ID  | Tier | Status | Item                                                                          | Source           | Notes                                                                                                                                                       |
|-----|------|--------|-------------------------------------------------------------------------------|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| B8  | T1   | Open   | NewPayAppModal: negative `Gross This Billing` / `Current Payment Due` + misleading "Less Previous Billings" placement | Found 2026-05-06 | T1 — embarrassing in front of paying customers; ~1h fix. When `THIS APP $` inputs are empty, Line 5 (Less Previous Billings) shows the prior-billed sum and Line 6 (Gross This Billing) goes negative (e.g. `-$43,091`); Line 8 follows. `validate()` blocks submit but the display is still scary. Math should clamp Line 6 ≥ 0 when nothing entered, OR Line 5 should be visually de-emphasized. File: `src/components/NewPayAppModal.jsx:106`. Repro: Plenium AGRU pay app on preview deploy `fix/restore-create-invoice-flow`. |
