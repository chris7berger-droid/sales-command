...
| H6    | H   | In Progress | `PublicSigningPage` `select("*")` exposes pricing internals                                       | Deep audit 2026-04-30           | Started 2026-05-05 on `fix/h6-public-signing-locked-totals`. Approach: store per-WTC total at lock time (new `proposal_wtc.locked_line_total`) so the public page reads `proposals.total` + `locked_line_total` via SECURITY DEFINER RPC instead of receiving cost basis / markup / materials. No SQL recompute — single source of truth stays in `src/lib/calc.js`. |
