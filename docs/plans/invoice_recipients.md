...
- **(b) Column-level boundary [the real boundary] — ⚠️ APPROACH CORRECTED POST-DEPLOY 2026-06-25, see "Post-deploy correction (#SEC1)" in Audit Amendments (post-T5):** ~~migration — `REVOKE SELECT (stripe_checkout_url, …) ON public.invoices FROM anon;`~~ **This column-level REVOKE is a SILENT NO-OP** — `anon` holds *table-level* SELECT on `invoices` (RLS needs it), and a table grant covers every column, so a column-level REVOKE carves nothing out of it. The working fix is **default-deny**: `REVOKE SELECT ON public.invoices FROM anon;` then `GRANT SELECT (<only the 15 columns PublicInvoicePage reads>) ON public.invoices TO anon;` (migration `20260625140000`). This makes `stripe_*`/`qb_*` physically unreadable by `anon` regardless of how a request is crafted (closes the hand-crafted-anon-request hole that (a) alone leaves open), and is default-deny for any future sensitive column.
  ...
  - **GOTCHA 2:** the GRANT must be a **column list** of only the safe columns — never `GRANT SELECT ON invoices TO anon` (blanket), which re-exposes everything. Keep the granted list in sync with `PublicInvoicePage.jsx`'s explicit select.
  - **GOTCHA 3 (the one that bit):** prove it with an **anon REST read** of a supposedly-blocked column before calling it done — blocked → `42501 permission denied`; a `200`/`[]` means the lock didn't take. A clean migration run and an audit sign-off are NOT proof.
...
**⚠️ Post-deploy correction (#SEC1) — 2026-06-25, during T3 deploy smoke:** the ratified part-(b) SQL (`REVOKE SELECT (cols) … FROM anon`) was applied to prod (migration `20260625130000`) and **did nothing** — an anon REST read of `stripe_checkout_url` still returned HTTP 200 afterward. Root cause: `anon` holds **table-level** SELECT on `invoices` (RLS needs it to return token-matching rows), and in Postgres a table-level grant covers every column, so a column-level REVOKE can't carve a hole out of it. Both T4 /buildvsplan and /security-review signed off on the migration text without catching this — it surfaced only because the deploy smoke included an explicit anon read. **Corrected approach (default-deny), applied via migration `20260625140000` and verified (anon `stripe_checkout_url` → `42501 permission denied`; the page's 15 columns → 200):** `REVOKE SELECT ON public.invoices FROM anon;` then `GRANT SELECT (<the 15 columns PublicInvoicePage reads>) ON public.invoices TO anon;`. §4.5(b) above updated to match. Durable lesson: a column-restriction migration is not "done" until an anon read of the blocked column is proven to fail — a clean run + audit sign-off is not proof. See memory `feedback-column-revoke-noop`.
