...
**Rebase note (2026-08-24):** branch rebased onto main after the screen-refresh merge (PR #36 etc.). Every mechanism the plan touches was re-verified intact; `WTCCalculator.jsx` line refs are UNCHANGED (verified: `:1239` collision const, `:1898` autosave, `:2191` handleSave, `:2257` handleLock, `:2299/:2313` proposalSold fetch, `:2425/:2444` overlay, `:2447-2452` tab gates). `ProposalDetail.jsx` refs drifted ~+13 lines — corrected anchors: `toggleWtcLock :312`, auto-create `:347`, `handlePullBack :569`, `handleInternalApprove :803` (body unchanged, still ungated), PDF-modal render + `onInternalApprove` prop `:847`, approve button `:922`, Send gate `:932`, + Add Work Type `:1048`. Read old ProposalDetail refs in §0/§3/§4/§5 with that offset; code is authoritative. **§0 population update (2026-08-24):** unlocked-committed count is now 22 (7 Sent / 1 Signed / 14 Sold) — one MORE proposal went Sold-unlocked since Aug 18, proving the gap is still firing; re-query at backfill time as specified.
