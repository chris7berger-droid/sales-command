...
_Generated by `/auditcriteria` on 2026-07-09 (round 3). Consumed by `/runaudit` to size the adversarial audit pass._

### Bottom line (plain English)
The big risk is gone. Rounds 1–2 hardened the design and then cut the one piece (Sales writing the mobilization table at send) that was generating almost all the findings — that's why round 2's severity collapsed. What's left is small and near-clean, so this is a light final check, not a deep one. Two reviewers: one confirms the slimmed-down handoff still gives the Schedule app everything it needs (the cut may have left the trip *labels and dates* off the wire), the other spot-checks that round-2's fixes are internally consistent. Expect few findings; do not manufacture more to fill the budget.

### Round
- Plan type: feature
- Current round: 3
- Plan revision under audit: 10fc7e3 (Plan revision pass 2 — round-2 audit response)
- Findings trend: round 1 (12: 1C/7H/4M) → round 2 (13: 0C/2H/8M/3L) → round 3 (?). **Count was flat 12→13 but severity COLLAPSED (1C/7H → 0C/2H), and the round-2 scope-cut removed the finding-carrying mechanism (the `job_mobilizations` seed + its ordered write/rollback/RLS surface). This is de-escalation, NOT a continuing plateau — the plateau was broken by cutting scope, exactly as the loop intends. Round 3 should come in materially lower.**

### Prior rounds
- Round 1: 72e66fd · 1C/7H/4M/0L · pattern: identity-by-position + asserted-not-implemented-write-discipline
- Round 2: 10fc7e3 · 0C/2H/8M/3L · pattern: plateau→scope-cut

**Briefing for agents**: do NOT re-find issues from prior rounds. The two revision-pass commit messages (72e66fd, 10fc7e3) are the canonical record of what was addressed — read them. Attack ONLY material new to revision 10fc7e3, i.e. **what the round-2 scope-cut changed or newly exposed**: the `stampDay` transform on both `field_sow` copies, the removal of the `job_mobilizations` seed (and what that leaves off the wire), the new self-fetch `useEffect`, and the `[K1]` fresh-fetch. Already-settled items — two-identity model, monotonic seq, delete-orphan scan, coercion map, NULL-date rendering, pull-back staleness — are LOCKED; do not relitigate.

**Plateau signal**: plateau forms when round-N count is steady or higher than round-(N-1) AND severity holds. Here severity fell hard and the mechanism was cut, so the plateau is considered **broken**. If round 3 nonetheless comes back at ≥8 findings or re-introduces High severity, that itself is the signal — and the only valid response is a further scope-cut, not another additive revision.
...
Agents weight severity against these values. Cross-tenant findings cap at Med while `live_tenants == 1`. Multi-user race findings cap at Low while solo. Theoretical attacks against state that doesn't exist yet (warehouse/crew tables Sales never writes; the `job_mobilizations` table Sales no longer writes) are not in scope and are not High.

### Time budget + finding cap
- **Time budget**: 120 min (ERD Loop #41; ratified with Chris 2026-07-09)
- **Finding cap**: 12 findings (formula: max(3, ceil(120/10)))

Synthesis MUST surface only the top-N most consequential findings. **Given the de-escalation, the realistic expectation is well under the cap (≤4) — the cap is a ceiling, not a target.** Remainder → "Quarantined findings (not actionable this loop)."

### Surface
- Total lines: 335 (plan body; excludes this manifest)
- Sections: 11 (§0–§10, plus §2A subsection)
- [LOCKED] decisions: 5 tagged `[LOCKED]` (scope boundary; §0.5; §1; §2; §3.3) + 4 RESOLVED decisions (D-IDENTITY two-identity; D1 editor location; D4=b scope-cut; D5 defer badge)
...
- Plan-to-code ratio: ~335 : ~300 est code lines ≈ 1.1:1 (well under 50:1; est code dropped with the cut — no seed, no ordered rollback)

### Layers touched (post-scope-cut)
- UI / components — mobilizations editor (ProposalDetail), mob selector + Sq Ft/Linear Ft inputs (WTCCalculator SowTab), pre-send review panel, new self-fetch effect
- State model — additive jsonb keys `mobilization_id`/`sq_ft`/`linear_ft` on `field_sow` day; `proposals.mobilizations` `{id,seq,label,start_date,end_date}` authoring
- Data layer — writes to `proposal_wtc.field_sow` (existing handleSave) + `proposals.mobilizations` (new); at send, the `stampDay` transform applied to BOTH `field_sow` copies (flat `jobs.field_sow` at :577/:604 and per-WTC `job_wtcs.field_sow` at :640). **No new write, no new row.**
- Cross-repo — Schedule (separate build) reads `mobilization_seq` off `job_wtcs.field_sow` and builds `job_mobilizations` itself; the day-key shape is the wire contract (§2A)
- **RLS / multi-tenancy — DROPPED as an attack layer this round.** The scope-cut removed the only new job-side write (`job_mobilizations` seed), so there is no new RLS surface, no service-role path, no new tenant-scoping to attack (§2 RLS note).

### New mechanisms introduced (post-cut)
- New jsonb keys: `mobilization_id text(uuid)|null`, `sq_ft numeric`, `linear_ft numeric` on each `field_sow` day (via `addDay`/`updateDay`, WTCCalculator.jsx:873/877)
- New authored jsonb: `proposals.mobilizations` entries `{id,seq,label,start_date,end_date}` (column live, currently unwritten in Sales)
...
- New self-fetch: `useState` + `useEffect` in WTCCalculator to load `proposals.mobilizations` by `proposalId` (useEffect-TDZ placement rule, §3.2 R1)
- New validation gate: `[K1]` pre-send — validated against a SEPARATE fresh `proposals.mobilizations` fetch at send (§5.1 R-E1)
- New helpers: `stampDay` (strip `mobilization_id`, stamp resolved `mobilization_seq`) applied to both copies; `DAY_COERCE` per-key map replacing the include-list
- **REMOVED by the cut** (no longer present — do not attack): `job_mobilizations` seed, the ordered `jobs→mob→job_wtcs` write, the cascade-backed rollback, the `jobs`-DELETE-RLS-policy dependency

### Cross-system reach
- **command-suite-db (shared DB)**: consumes the LOCKED plan0 schema — read-only; Sales writes neither the schema nor `job_mobilizations` now.
- **Schedule Command (future build, same DB)**: reads `mobilization_seq` + the metric keys off `job_wtcs.field_sow`, then seeds `job_mobilizations` itself. **The seam is now seq-only on the wire** — see weak point #1 (label/dates gap).
- **Service-role / bypass paths**: none.

### Irreversibility
- none — no DDL, no backfill, no new write, no public API change. All changes additive jsonb / value-only stamps; the cross-repo key-shape is soft (renaming later breaks Schedule) but reversible in-repo.

### Known weak points (attack these — new to the scope-cut)
- **#1 (highest pressure) — the cut may have stripped mobilization label/dates off the wire.** Under the old D4=a seed, Sales wrote full `job_mobilizations` rows `{seq,label,start_date,end_date}`. Under D4=b, the send stamps ONLY `mobilization_seq` onto `field_sow` days (§5.3 `stampDay`). The mob **label + start/end dates now live only on `proposals.mobilizations`.** §2A says "Schedule seeds `job_mobilizations` from the `mobilization_seq`" — but seq alone can't reconstruct label/dates. Does Schedule read `proposals.mobilizations` cross-driver (Schedule reaching into Sales' proposal table — is that a sanctioned pattern in the Command Suite data contract?), or is the handoff genuinely incomplete? This is the sharpest gap the cut introduced; the plan does not resolve where Schedule gets mob metadata.
- **#2 — `stampDay` must be applied identically to BOTH `field_sow` copies (C1).** The flat `jobs.field_sow` (merge at :577) and the per-WTC `job_wtcs.field_sow` (:640) are built at two separate sites. If the build stamps only one, the copies diverge on `mobilization_seq` and a reader of the flat mirror sees no mobilization. `seq` is now the SOLE wire carrier, so a missed site = silent data loss, not a redundant copy.
- **#3 — self-fetch `useEffect` correctness (R1).** New `useState`+`useEffect` in WTCCalculator; the plan specs placement AFTER the `useState` block (useEffect-TDZ rule) and a `mobsLoaded` flag to disambiguate "loading" from "no mobs". Verify the spec is internally consistent (e.g. does `addDay`'s `mobilizations[0]?.id` default interact correctly with the gate that disables "+ Add day" until loaded?).
- **#4 — `[K1]` two-fetch freshness (R-E1).** `[K1]` validates `proposal_wtc.field_sow` (re-fetched at :569) against a SEPARATE new `proposals.mobilizations` fetch. Confirm both are genuinely fresh reads (not stale `ProposalDetail` state) and that the resolution map is built from the same fetch `[K1]` validated against — a split between "validated set" and "stamped set" would let an unresolved day through.

### Open questions
- Count: 2 (§9 — D2 day-card layout, D3 review-panel depth; both build-time, non-blocking — do NOT spend budget here).
- Highest-pressure (NOT in §9, surfaced by the cut): **where does Schedule get mobilization label/dates?** (weak point #1). This is a cross-driver contract question the plan leaves open; worth one agent's focused attention, but frame it as "is the handoff complete," not "redesign the cut."

### Suggested attack angles (2 total)
1. **Post-cut handoff completeness + wire-contract correctness** — covers cross-repo + data layer + state model. Required reading: §2/§2A, §5.3 (`stampDay`, both stamp sites), `ProposalDetail.jsx:577` (flat merge) + `:640-660` (per-WTC job_wtcs), plan0 `job_mobilizations` shape (`20260708120100_*.sql`). Specific pressure: (a) weak point #1 — with only `seq` on the wire, can Schedule actually build `job_mobilizations` (label/dates), or is the handoff incomplete? Is Schedule-reads-`proposals.mobilizations` a sanctioned cross-driver pattern or an undeclared dependency? (b) weak point #2 — are BOTH `field_sow` copies stamped identically by `stampDay`, and is `mobilization_id` actually stripped from both? (c) Does stamping `seq` (not `id`) match what the LOCKED plan0 contract says keys the day tag?
2. **Sales-side authoring correctness (round-2 fixes hold-up)** — covers UI + state model + the `[K1]` gate. Required reading: §3.1 (`DAY_COERCE`), §3.2 (self-fetch `useEffect` + "+ Add day" gating), §4 (editor: monotonic seq, delete in-use scan), §5.1 (`[K1]` two-fetch). Specific pressure: verify the round-2 fixes are internally consistent, not just individually plausible — weak points #3 and #4 above. Does the self-fetch/gate/default trio interact cleanly (TDZ placement, `mobsLoaded` ambiguity, `mobilizations[0]?.id` default)? Can `[K1]`'s "validated set" and the send's "stamped set" ever diverge? Is there any authoring sequence that strands a proposal un-sendable that the delete-scan doesn't warn about?

### Suggested agent count: 2

Rationale: the raw formula scores 6 (4 layers + cross-system + ≥3 mechanisms) → capped 5, but that formula is blind to round history. This is round 3 AFTER a scope-cut that collapsed severity (1C/7H → 0C/2H) and removed the entire RLS/write-path/rollback surface — over-staffing a de-escalated near-clean plan re-manufactures the plateau the cut just broke (the Loop #28 lesson). Two aimed agents cover the only genuinely new surface (handoff completeness) plus a consistency check on the round-2 fixes; a 3rd would have nothing left to attack but the LOCKED settled decisions.
