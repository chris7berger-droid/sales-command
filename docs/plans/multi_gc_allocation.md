...
3. **Client fallback pattern rollout.** Replace `call_log?.customer_id` with `customer_id ?? call_log?.customer_id` everywhere in src/. Three known sites: ProposalDetail.jsx:62, :282; ProposalPDFModal.jsx:31. Plan agent's prior delivery flagged "Customers.jsx + 4 other files" — re-grep to be sure. Update `get_public_proposal_view` RPC to COALESCE customer chain (§11 v98 finding still valid).
   ...
   **A1.1 Extension — 2026-05-14 — additional sites surfaced during §10 Step 4 audit.**

   The §10 Step 4 audit (2026-05-14, Pass 3 wider-site sweep) ran the build-time grep recommended in §A1.5 above. The sweep surfaced two additional sites in the same A1.1 class as the original four: both pre-populate customer-derived state from `call_log.customers` only, ignoring `proposals.customer_id`. For a sister proposal where `proposals.customer_id` differs from `call_log.customer_id`, both load the WRONG GC's customer data.

   - `src/components/NewPayAppModal.jsx:39` — `supabase.from("call_log").select("customer_id, subcontractor_job_no, job_number").eq("id", proposal.call_log_id).maybeSingle()`. The returned `customer_id` flows to `:67-72` for pay-app template fetch (`from("customer_pay_app_templates").eq("customer_id", customerId)`). For a sister GC, this loads the parent's templates instead of the sister's.
   - `src/components/PayAppDetailModal.jsx:66` — `supabase.from("call_log").select("customer_id, job_number, subcontractor_job_no, customers(*)").eq("id", proposal.call_log_id).maybeSingle()`. Same shape; the embedded `customers(*)` flows downstream into `:80-92` billing-contact derivation + `:103-110` template lookup. Pay-app render for a sister GC shows the parent's customer billing data and templates.

   **[LOCKED] Fix shape — same as original A1.1.** Both modals re-shape the call_log fetch to include the parallel `customers` embed via `proposals_customer_id_fkey`, or (preferred) consume `useResolvedCustomer(p)` per Step 3 Ratification #1. Resolution: `const customer = proposal.customers ?? proposal.call_log?.customers`. **No new architectural decision**; mirrors the four original A1.1 sites exactly. Both sites take the embed at the `proposals` select that loads the pay-app, OR re-shape the `call_log` fetch to a `proposals` fetch with `customers` + `call_log(customer_id, customers(*))` embeds and apply the `??` rule.

   **Site count delta.** Original A1.1 inventory = 4 sites (`send-invoice`, `send-pay-app`, `ProposalDetail`, `ProposalPDFModal`). With this extension = **6 sites total.** Step 3 build session adds the two pay-app modals to the patch list. **Does not edit upstream A1.1 text** per [Schema Amendment Not Overwrite] — the original 4-site inventory and this 2-site extension together are the authoritative full set.

   **Verification.** Identified via 2026-05-14 audit grep: `grep -rnE 'call_log.*customer_id|call_log\\..*customers' src/` ⇒ all 6 sites enumerated; remaining matches across `src/` are either mutations, contact lookups, or single-row id-based reads that don't suffer the S1 regression. Sweep clean beyond these 6.
