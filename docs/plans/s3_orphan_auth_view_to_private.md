...
| 3 | New view uses security_invoker | Supabase SQL: `SELECT reloptions FROM pg_class WHERE relnamespace = 'private'::regnamespace AND relname = 'v_orphan_auth_users';` | array containing `security_invoker=true` |
| 4 | Grant set is minimal | Supabase SQL: `SELECT grantee, privilege_type FROM information_schema.role_table_grants WHERE table_schema='private' AND table_name='v_orphan_auth_users';` | only `service_role` (no `anon`, no `authenticated`, no `public`) |
