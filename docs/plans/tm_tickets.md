...
The plan got smaller, not bigger — the rewrite deleted three new tables and a whole screen instead of adding fixes on top. What's left is mostly one risky thing: replacing a live database function that serves the page your customers sign, with a safety check that currently can't see whether it worked. Three reviewers, six problems back.

### Round
- Plan type: **feature**
- Current round: **2**
- Plan revision under audit: `60605fc`
- Findings trend: `round 1 (23 caused-by / 6 top: 1C/5H) → round 2 (?)`

**Read the trend carefully this round.** The v1→v2 revision **removed** mechanism — 3 tables, 12 RLS policies, a modal, 2 routes, a numbering allocator, a unique index. That is the opposite of the usual plateau shape, where each revision answers findings by adding surface. If round 2 comes back flat or higher against a materially smaller surface, that is a genuine signal the model is wrong. If it drops, the model change worked and the loop should close.

### Prior rounds
- Round 1: `b29a3b1` · 23 caused-by (6 top / 17 over cap) + 5 adjacent · **1C/5H/15M/4L** · pattern: `rate-card-exclusion-overreach`

**Briefing for agents**: do NOT re-find issues from prior rounds. `60605fc`'s commit message is the canonical record of what was addressed. Round-1 findings **F, A2, E** and the double-billing index, ticket routes, ticket numbering, the 3 tables and 12 policies are **structurally gone** — the objects they attacked are not built. **A1, A3, B, C, D** are answered in §5.4, §5.5, §4.4, §4.2, §7 — attack whether those answers are *correct*, not whether the problems existed.

**Plateau signal**: plateau forms when round-N count is steady or higher than round-(N-1). `/runaudit` MUST present scope-cut as the only build-prompt option when plateau is detected. Hedged prompts make the loop worse.

### Deployment context
- **Live tenants**: **1** — HDSP (`246f6551-60de-4965-bb97-9a52971bc05d`, run-verified). Multi-tenant onboarding blocked.
- **Prod / staging / dev**: every **modified** surface is live in prod for a paying customer — proposal totals, the proposal PDF, the **public signing page**, invoice creation and editing, the public invoice page, the job screen. The only net-new things are 13 columns with no rows.
- **Blocking feature flags**: none. `requires_pay_app = false` on Contract Flooring keeps job 7215 on the regular-invoice path.
- **Concurrency profile**: **solo** — one office user.

Cross-tenant findings cap at Med while `live_tenants == 1`. Multi-user race findings cap at Low while solo. Theoretical attacks on state that doesn't exist yet are not High.

### Time budget + finding cap
- **Time budget**: **60 min** (Chris, 2026-08-07; drafter's post-rewrite estimate 180 — §8)
- **Finding cap**: **6** findings

Remainder go to "Quarantined findings (not actionable this loop)."

**Do not report unbuilt scope as a finding.** §8 records an accepted estimate divergence with scope deliberately preserved.

### Surface
- Total lines: 518 (v1 was 677)
- Sections: 12 (§0–§11)
- [LOCKED] decisions: 17
- [DESIGN-OPEN] items: 4
- [OPEN] items: 5 (§10)
- Plan-to-code ratio: **518 : 900** ≈ 0.58:1 — well under the 50:1 flag.

### Layers touched
- UI / components — `WTCCalculator`, `Invoices`, `PublicInvoicePage`, `PublicSigningPage`, `CallLogDetail`, `ProposalPDFModal`, `ProposalDetail`, `MultiGCWizard`
- Data layer — invoice-line loaders and embeds; a **new** line-level load on the job screen (§6, uncosted — see weak points)
- State model — 13 additive columns; stored-not-derived `amount`
- Migrations / schema — Migration A (columns) + **Migration B (`CREATE OR REPLACE` on a `SECURITY DEFINER` function)**
- Auth / anon exposure — the signing-page RPC is the app's audited anon boundary (H6)
- Cross-repo — `command-suite-db` forward-only ledger shared with `sch-command`, `field-command`
- External integrations — QuickBooks (inherits the T&M description via §3.3 with no edge-function change — verify)

**No RLS layer this round.** No new tables, no new policies; existing `proposal_wtc` / `invoice_lines` / `invoices` policies cover the new columns. This was one of round 1's four angles and is now moot.

### New mechanisms introduced
- New columns: `proposal_wtc` ×3 (§2.1), `invoice_lines` ×9 (§3.1), `invoices.nte_amount` ×1 (§5.3)
- New tables / policies / triggers / indexes / routes: **none**
- **Modified `SECURITY DEFINER` RPC**: `get_public_proposal_view` (§4.6)
- **Behavior change to a shared function**: `calcProposalTotal` gains a rate-card filter (§4.2) — every caller inherits it
- **Behavior change to a shared function**: `sumContractBilled` gains a rate-card filter (§6) — see weak points, may not be implementable as written
- New shared helper: the proposal-total recompute extracted for both `toggleWtcLock` and `WTCCalculator.handleSave` (§4.4) — **invented, adds a second writer to `proposals.total`**
- New UI mode: T&M day-row form inside the existing invoice modal (§5.2)
- New preserve branch in `handleSaveEdit` (§5.4)
- New procedural mechanism: a **behavioural** key-diff check for Migration B (§7 step 2) — specified in prose, not as a script
- New rollback file + a hand-edited rehearsal baseline refresh (§7)

### Cross-system reach
- `command-suite-db` — forward-only ledger shared across three repos; two migrations in one push
- Shared Supabase project `pbgvgjjuhnpsumnowuym` — Field and Schedule read the same database
- **Anon / unauthenticated**: `PublicSigningPage` (via the RPC) and `PublicInvoicePage` (via PostgREST with an anon key)
- QuickBooks via `qb-sync-invoice` — service-role reader of `invoice_lines`
- Field devices receive the 3 new `proposal_wtc` columns through a `SELECT *` sync rule (round-1 adjacent, Low)

### Irreversibility
- **Migration A**: additive columns only — reversible.
- **Migration B**: `CREATE OR REPLACE` on a live `SECURITY DEFINER` function serving the **unauthenticated** signing page, no flag, no staged rollout. Highest-consequence step in the build.
- **The rehearsal baseline itself is rewritten** (§7 step 1) including a hand-edit `db dump` cannot emit. This changes the safety net, not just the thing under it.
- **Shared-function behavior changes** — `calcProposalTotal` and `sumContractBilled` change for every caller at once.
- **`proposals.total` rewritten for P7**, a `Sent` proposal already in a customer's hands.
- **P7 is marked Sold** (§9 step 0) — a business state change on a live proposal, taken to unblock a build step.

### Known weak points

- **§4.2 oversells the aggregator fix. `calcProposalTotal` has exactly ONE caller** — `MultiGCWizard.jsx:221` [read-verified]. The other seven sum-sites (S1–S5, S7, S8) hand-roll their own reduce and do **not** go through it. So "fix the aggregator and every caller inherits it" is true of one site out of eight; the remaining seven must each be routed through it or filtered in place. The plan says this in a parenthesis and then frames it as solved. **This is the same class of over-claim that produced round-1 finding C.**
- **§6's `sumContractBilled` filter is not implementable as specified.** `sumContractBilled` takes **invoices** and sums `i.amount` (`calc.js:203-208`). Excluding "dollars on rate-card lines" requires line-level data. `CallLogDetail.jsx:222` loads invoices with `select("id, status, amount, job_name, voided_at, void_reason, retention_release_of")` — **no lines at all** [read-verified]. An invoice mixing material percent lines and T&M day rows has one `amount` and cannot be split without a new query.
- **§6's T&M row needs data the job screen doesn't load.** Hours-by-class across the job requires every invoice's day rows. Not in the current load, not costed in §8.
- **§4.4 creates a second writer to `proposals.total`.** `toggleWtcLock` and `WTCCalculator.handleSave` would both write it on different triggers. The plan calls for a shared function, which prevents *formula* drift but not two writers disagreeing about *when*.
- **§5.4's fix depends on an embed the plan mentions once.** The preserve branch tests `l.proposal_wtc?.is_rate_card`; if the embed at `Invoices.jsx:1541` is missed, the test is silently false and the Critical returns unchanged. A silent-false guard on a money path deserves more than a sentence.
- **§9 step 0 marks P7 Sold with no impact analysis.** Selling a proposal in this app has downstream effects (QuickBooks customer/job creation, schedule handoff). The plan treats it as "a data step, not code." **Verify what marking a Sent proposal Sold actually triggers** before it is done to a live customer's proposal.
- **§7's behavioural check is prose, not a script.** Who runs it, when, and does it gate the push? Round-1 finding D was that the harness passes vacuously; a check that isn't wired can pass vacuously too.
- **§3.1 — no check constraint ties `amount` to `hours × rate`.** A UI arithmetic bug writes a wrong billed dollar with nothing to catch it. §5.2's live-total-against-the-paper is the only mitigation and it depends on a human comparing two numbers.
- **§3.3's safety claim is asserted broadly.** T&M lines carrying a rate-card WTC is safe *for `getBilledPct`* because `billing_pct` is null. Anything else grouping by `proposal_wtc_id` and assuming one line kind per WTC now sees two.
- **§3.1 — prevailing-wage T&M does not fit.** `proposal_wtc.prevailing_wage`, `pw_rate`, `pw_ot_rate` already exist; three fixed slots cannot express a PW split. Named as a limit (O5) — confirm it's genuinely acceptable.
- **§5.3 — the NTE is per-invoice only.** If a GC ever gives a job-level ceiling there is nowhere to put it. Accepted under L7; named so the decision stays visible.

### Open questions
- Count: **5** (§10: O1–O5)
- Highest-pressure: **O5** (prevailing-wage T&M doesn't fit the three fixed slots — the only open item that would force DDL later) and **O2** (which rate card a mixed-class day points at)

### Suggested attack angles (3 total)

1. **Migration B + the safety harness** — covers migrations, auth/anon exposure, cross-repo. Required reading: `src/pages/PublicSigningPage.jsx:1-20, 540-560`, the live `get_public_proposal_view` body (§4.6 quotes it), `command-suite-db/scripts/rehearse.sh`, `scripts/check-migration-safety.sh`, `command-suite-db/supabase/rollbacks/`. Specific pressure: does §7's baseline refresh + behavioural key-diff actually close round-1 finding D, or is it prose that will pass vacuously? Is `CREATE OR REPLACE` on this function safe with no flag on the unauthenticated signing path — what is the rollback in practice, and how fast? Does exposing `rate_amount` reopen H6 (verify, don't accept the plan's claim)? Does refreshing the baseline (with a hand-edit `db dump` can't emit) weaken the net for everything else?

2. **Money model** — covers state model, business logic, data layer. Required reading: `src/lib/calc.js:180-210`, `src/components/ProposalDetail.jsx:320-360, 1260-1280, 885-914` (job totals in `CallLogDetail.jsx`), `src/components/MultiGCWizard.jsx:221, 520-640`, `src/pages/Invoices.jsx:100-125`. Specific pressure: **the aggregator fix reaches one of eight sites** — prove which of S1–S8 actually end up excluded and which are still hand-rolled. **§6's `sumContractBilled` filter is not implementable as written** (invoice-level sum, no lines loaded) — what is the real design? What does the T&M row cost in queries? Does the second writer to `proposals.total` (§4.4) race or disagree with `toggleWtcLock`?

3. **Invoice-line lifecycle** — covers UI, data layer, external integrations. Required reading: `src/pages/Invoices.jsx:160-320, 940-950, 1530-1560, 1740-1860, 2120-2150, 2350-2380`, `src/pages/PublicInvoicePage.jsx:35-60, 215-235`, `supabase/functions/qb-sync-invoice/index.ts:140-260`. Specific pressure: trace one T&M line end to end — create, display, edit, void-and-replace, public page, QuickBooks. Does §5.4's preserve branch survive a missed embed at `:1541`? Does §5.5 carry all nine columns? Is §3.3's "carries the rate-card WTC" genuinely safe at **every** site that groups by `proposal_wtc_id`, or only at `getBilledPct`? Does QuickBooks really inherit the right description with no edge-function change?

### Suggested agent count: 3

Rationale: the formula yields 10 angles (7 layers + cross-system + ≥3 novel mechanisms + ≥5 open questions) and would cap at 5, but the surface genuinely shrank — the RLS angle that consumed one of round 1's four is moot with no new tables or policies, and the ticket-object angle has nothing left to attack. Three angles cover every live prod surface against a 6-finding cap without paying for overlap. Going to 4 would mean splitting angle 3, which is one coherent trace and reads better whole.
