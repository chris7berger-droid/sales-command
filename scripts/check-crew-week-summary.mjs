...
try {
  ...
  await badge('Jobs Starting').click()
  await page.getByRole('dialog').getByText('October sealing',{exact:true}).waitFor()
  await page.getByRole('dialog').getByText('Check overlap',{exact:true}).waitFor()
  await page.getByRole('dialog').getByRole('button',{name:'Close',exact:true}).click()
  ...
  await page.locator('.sch-brd-job-label').click()
  ...
  console.log('PASS shared weeks, all controls, deep links, glow/reduced motion, trip counts/details, daily people, loading/races, error/retry, and unchanged Logistics header.')
