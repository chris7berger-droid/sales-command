...
assert.match(result.text, /SUNDAY, SEP 13[\s\S]*Start: Delayed start 1:15 PM/)
assert.doesNotMatch(result.text, /With:/)
assert.match(result.text, /CREW[\s\S]*6618 — Final Burnish[\s\S]*JoseJR[\s\S]*Kurtis/)
assert.match(result.text, /Wrong Day/)
assert.doesNotMatch(result.text, /Wrong Trip|PRIVATE OFFICE|Other Lead|Parent Lead/)
assert.doesNotMatch(result.text, /No work assigned/)
assert.doesNotMatch(result.text, /MONDAY, SEP 7|TUESDAY, SEP 8|WEDNESDAY, SEP 9|THURSDAY, SEP 10|SATURDAY, SEP 12/)
assert.equal(result.days.length, 2)
assert.equal(result.warnings.length, 0)
const noLeadAlloc = { 1: { 1: { id: 'burnish', seq: 1, label: 'Final Burnish', start_date: dates[4], end_date: dates[6] } } }
const inherited = buildCrewWeekText({ ...args, allocations: noLeadAlloc })
assert.match(inherited.text, /Lead: Parent Lead/)
assert.doesNotMatch(inherited.text, /Kurtis Zomparelli|Confirm with office/)
const noLead = buildCrewWeekText({ ...args, jobs: [{...job, lead: ''}], allocations: noLeadAlloc })
assert.match(noLead.text, /Lead: Confirm with office/)
assert.ok(noLead.warnings.some(w => /lead needs confirmation/.test(w)))
const ambiguous = buildCrewWeekText({ ...args, assignments: [{job_id:1,date:dates[4],crew_name:'JoseJR'}] })
assert.match(ambiguous.text, /Lead: Parent Lead/)
...
assert.match(parent.text, /Lead: Parent Lead/)
assert.doesNotMatch(parent.text, /With:/)
assert.match(parent.text, /Parent Coworker/)
...
assert.doesNotMatch(empty.text, /No work assigned|MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY|SATURDAY|SUNDAY/)
...
console.log('PASS: day/trip details, crew grid, override/inheritance, Sunday/DST, deferred start, missing jobs, board lead on unlinked days, skip empty days, duplicate/multiple assignments, internal note exclusion')
