...
await page.route('**/*', async route => {
  ...
  if (table === 'check_schedule_deletion') {
    const { p_job_id, p_trip_id } = req.postDataJSON()
    assert.equal(p_job_id, 1150)
    const blocked = assignments.some(a => !p_trip_id || !a.mobilization_id || a.mobilization_id === p_trip_id)
    return send(blocked ? { code: 'crew', message: 'Cannot delete this trip — crew is still assigned. Remove or reassign all crew in Crew Schedule first.' } : null)
  }
  if (req.method() !== 'GET') {
...
try {
  ...
  if (process.env.TRIPS_SNAPSHOT) {
    ...
    await page.getByRole('textbox', { name: 'Trip title', exact: true }).waitFor()
    const originalTitle = await page.getByLabel('Trip title', { exact: true }).inputValue()
    await page.getByLabel('Trip title', { exact: true }).fill('   ')
    await page.getByRole('button', { name: 'Save', exact: true }).click()
    await page.getByText('Enter a trip title before saving.', { exact: true }).waitFor()
    assert.equal(mutations.length, 0, 'Blank trip title must not write anything')
    await page.getByLabel('Trip title', { exact: true }).fill(originalTitle)
    ...
    failSave = false
    await page.getByRole('button', { name: 'Save', exact: true }).click()
    await page.getByRole('textbox', { name: 'Trip title', exact: true }).waitFor({ state: 'hidden' })
    ...
    await page.getByLabel('Trip title', { exact: true }).fill('Warranty check')
    ...
    await page.waitForFunction(() => !document.querySelector('[aria-label="Trip title"]'))
    ...
    await page.locator('.job-schedule-reference').waitFor()
    assert.match(await page.locator('.job-schedule-reference').innerText(), /Job schedule/)
    assert.match(await page.locator('.job-schedule-reference').innerText(), /Oct 12, 2026 – Oct 30, 2026/)
    assert.equal(await page.locator('.job-trip').count(), 0)
    assignments = ['2026-10-12', '2026-10-30'].map((date, i) => ({ id: i + 1, job_id: 1150, crew_name: 'Bash Dave', date, mobilization_id: null }))
    await page.getByRole('button', { name: 'Refresh trips' }).click()
    await article('legacy:2026-10-12').waitFor()
    await article('legacy:2026-10-30').waitFor()
    assert.equal(await page.locator('.job-trip').count(), 2, 'Unlinked days stay separate from the job date summary')
    await page.setViewportSize({ width: 1440, height: 1050 })
    await page.screenshot({ path: '/private/tmp/codex-review-parent-trip.png', fullPage: true })
    console.log('PASS parent-only reference dates remain unchanged; unlinked crew days never imply trip ownership.')
    ...
    assert.equal(await page.locator('.job-trip').count(), 3)
    assert.equal(await page.locator('.job-trip[data-trip-id^="job:"]').count(), 0, 'Job date summary is not a trip card')
    assert.equal(job.start_date, '2026-10-12')
    assert.equal(job.end_date, '2026-10-30')
    ...
    console.log('PASS saved trip replaces synthetic plan; original dates are unchanged and unlinked crew remains visible separately.')
  }
  if (!process.env.TRIPS_SNAPSHOT) {
    ...
    const warning = page.getByRole('alertdialog', { name: 'Delete trip' })
    await warning.getByText('Cannot delete this trip', { exact: true }).waitFor()
    assert(rows.some(r => r.id === 'short'))
    assert.equal(await warning.getByRole('button', { name: 'Continue' }).count(), 0)
    await warning.getByRole('button', { name: 'Cancel', exact: true }).click()
    // Resolve both this trip's crew and the explicitly unlinked legacy days.
    assignments = []
    await page.getByRole('button', { name: 'Delete trip', exact: true }).click()
    await warning.getByRole('button', { name: 'Continue' }).waitFor()
    await warning.getByRole('button', { name: 'Cancel', exact: true }).click()
    assert(rows.some(r => r.id === 'short'), 'Cancel preserves trip')
    await page.getByRole('button', { name: 'Delete trip', exact: true }).click()
    await warning.getByRole('button', { name: 'Continue' }).click()
    assert(await warning.getByRole('button', { name: 'Permanently delete trip' }).isDisabled())
    await warning.getByLabel('Type DELETE to confirm').fill('DELETE')
    await warning.getByRole('button', { name: 'Permanently delete trip' }).click()
    await warning.waitFor({ state: 'hidden' })
    await page.getByRole('dialog').waitFor({ state: 'hidden' })
    await article('short').waitFor({ state: 'hidden' })
    assert.equal(await page.locator('.job-schedule-reference').count(), 1, 'Job date summary remains outside the trip list')
