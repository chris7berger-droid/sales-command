...
try {
  ...
  if (process.env.TRIPS_SNAPSHOT) {
    ...
    assert.match(await initial.innerText(), /No crew assigned yet/)
    assert.equal(await page.locator('.job-trip').count(), 3, 'Unlinked days stay separate from the parent plan')
    ...
    assert.equal(await initial.count(), 0, 'Saved trips replace the synthetic parent plan')
