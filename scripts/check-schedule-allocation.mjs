...
async function modal(){
  ...
  await page.getByPlaceholder('Trip title (required)').fill('WTC1 - Concrete Sealing')
...
try{
 ...
 await page.getByText('Enter a trip title before adding it.', {exact:true}).waitFor()
 assert.equal(mutations.length,0,'Blank title cannot create a trip or any other record')
 await page.getByLabel('Trip title', {exact:true}).fill('WTC1 - Concrete Sealing')
 ...
 failInsert=false
 await lateForm.getByRole('button',{name:'Save trip',exact:true}).click()
 await lateForm.getByRole('button',{name:'Cancel',exact:true}).waitFor({state:'hidden'})
 ...
 await details.getByRole('button',{name:'Cancel',exact:true}).waitFor({state:'hidden'})
 assert.equal(await details.getByLabel('Trip notes',{exact:true}).inputValue(),'Updated from Crew Schedule')
 ...
 await create.getByLabel('Trip title',{exact:true}).fill('December trip')
 ...
 await create.getByLabel('Trip title',{exact:true}).waitFor()
