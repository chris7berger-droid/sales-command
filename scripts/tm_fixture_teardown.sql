...
--
-- Removes job 999 and everything hanging off it. Run when the build is done;
...
declare v_id bigint;
begin
  select id into v_id from public.call_log where job_number = 999;
  if v_id is null then
    raise notice 'Fixture job 999 not found — nothing to tear down.';
  elsif v_id <> 3810 then
    raise exception 'Refusing: job_number 999 resolves to call_log.id=% (expected 3810). Investigate before deleting anything.', v_id;
  end if;
end $$;
...
delete from public.call_log       where id = 3810 and job_number = 999;
