    ...
    if (sch) {
      const [{ data: lns }, { data: apps }, { count }] = await Promise.all([
        ...
        supabase.from("billing_schedule_pay_apps")
          .select("id, app_number, period_from, period_to, this_app_amount, retainage_withheld, current_payment_due, status, pdf_url, sov_pdf_url, invoice_id, created_at, submitted_at")
          .eq("billing_schedule_id", sch.id)
    ...
    <div style={card}>
      ...
      {payApps.length > 0 && (
        <div style={{ marginTop: 18 }}>
          ...
          <div style={{ border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 120px 120px 120px 120px 110px 110px", background: C.dark, padding: "8px 12px", gap: 10 }}>
              {["#", "Period", "This App", "Retention", "Payment Due", "Invoice", "Sent", ""].map((hh, i) => (
                <div key={i} style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.5)", fontFamily: F.display, letterSpacing: "0.08em", textTransform: "uppercase", textAlign: i === 0 || i === 1 ? "left" : i === 7 ? "center" : "right" }}>{hh}</div>
              ))}
            </div>
            {payApps.map(pa => {
              const isSent = pa.status === "submitted" || pa.status === "paid";
              return (
              <div key={pa.id} onClick={() => setDetailPayAppId(pa.id)} style={{ display: "grid", gridTemplateColumns: "60px 1fr 120px 120px 120px 120px 110px 110px", padding: "8px 12px", gap: 10, borderTop: `1px solid ${C.border}`, alignItems: "center", background: C.linenLight, cursor: "pointer" }}>
                ...
                <div style={{ fontSize: 11, color: C.textFaint, fontFamily: F.ui, textAlign: "right" }}>
                  {pa.submitted_at ? new Date(pa.submitted_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "—"}
                </div>
                <div style={{ textAlign: "center" }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: C.teal, fontFamily: F.display, letterSpacing: "0.05em", textTransform: "uppercase", background: C.dark, padding: "3px 9px", borderRadius: 4 }}>
                    {isSent ? "Review Package" : "View Data"}
                  </span>
                </div>
              </div>
              );
