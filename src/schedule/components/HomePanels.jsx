  ...
  return (
    <section className="hp-card hp-needs">
      ...
      <div className="hp-alert-list">
        {rows.map(r => (
          <button key={r.key} className="hp-alert-row" onClick={() => navigate('/schedule/jobs?tab=all')}>
      ...
      <button className="hp-footer-link" onClick={() => navigate('/schedule/jobs?tab=all')}>View All Alerts →</button>
    </section>
  )
}
...
export function NextUp({ nextUp }) {
  ...
  const goCrewSchedule = () => {
    ...
    if (s) {
      navigate(`/schedule/schedule?job=${job.job_id}&week=${fmtD(getMonday(new Date(s + 'T00:00:00')))}`)
    } else {
      navigate(`/schedule/schedule?job=${job.job_id}`)
    }
  }
...
export function AtAGlance({ data }) {
  ...
  return (
    <section className="hp-card hp-glance">
      ...
      <button className="hp-footer-link" onClick={() => navigate('/schedule/billing/forecast')}>View Analytics →</button>
    </section>
  )
}
