...
export default function JobsPicker({ jobs = [], assignments = [], billingWorklist = [], crewByCallLog = {}, matsByJobId = {}, syncWarning, today = new Date(), onPick, onOpenBin }) {
  ...
  const goTab = (key) => onPick ? onPick(key) : navigate(`/schedule/jobs?tab=${key}`)
  const goSchedule = () => navigate('/schedule/schedule')
  const goBilling = () => navigate('/schedule/billing')
  const goForecast = () => navigate('/schedule/billing/forecast')
  const goProductionRate = () => navigate('/schedule/production-rate')
  const goBudget = () => navigate('/schedule/budget')
  const goDaily = () => navigate('/schedule/daily')
  ...
  return (
    <div className="jh-picker">
      <div className="jh-picker-intro">
        ...
        {onOpenBin && (
          <button className="jh-bin-btn" onClick={onOpenBin} title="Recover jobs deleted in the last 24 hours">{'🗑'} Recovery Bin (24 hrs)</button>
        )}
      </div>
