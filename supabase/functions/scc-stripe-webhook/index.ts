...
serve(async (req) => {
  ...
  try {
    ...
    // S8 CLAIM: record event.id before processing. PK conflict (23505) = seen before.
    const claim = await supabase
      .from("processed_stripe_events")
      .insert({ event_id: event.id, event_type: event.type, source: "scc-stripe-webhook", status: "claimed" });
    if (claim.error) {
      if (claim.error.code === "23505") {
        ...
        if (existing?.status === "done") {
          console.log("Duplicate SCC Stripe event, already processed:", event.id);
        ...
        console.log("Re-processing previously-claimed SCC Stripe event:", event.id);
      } else {
    ...
    // Covers every handled subscription branch AND any unhandled event type
    // (which falls through to here) — every 2xx is marked done.
    await markDone();
