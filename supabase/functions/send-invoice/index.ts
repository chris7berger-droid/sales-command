  ...
  try {
    ...
    const stripeRes = await fetch("https://api.stripe.com/v1/checkout/sessions", {
      ...
      body: new URLSearchParams({
        ...
        "line_items[0][price_data][product_data][name]": `Invoice #${invoiceId}${jobName ? ` — ${jobName}` : ""}`,
        "line_items[0][price_data][product_data][description]": `${jobId ? `Job #${jobId} · ` : ""}High Desert Surface Prep`,
