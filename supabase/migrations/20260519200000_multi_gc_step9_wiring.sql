-- Step 9 — Multi-GC wizard RPC wiring
-- F18: Add rfp_number, bid_due_date, billing_terms_override to proposals
-- F19: Update clone_proposal_to_gcs to handle intro_override, signer/viewer
--      contacts, rfp_number, bid_due_date, billing_terms_override

-- ============================================================================
-- F18 — Schema additions
-- ============================================================================

ALTER TABLE public.proposals
  ADD COLUMN IF NOT EXISTS rfp_number text,
  ADD COLUMN IF NOT EXISTS bid_due_date date,
  ADD COLUMN IF NOT EXISTS billing_terms_override integer;

-- ============================================================================
-- Updated clone_proposal_to_gcs
-- ============================================================================
-- Now reads from each target: customer_id, markup_override_pct, rfp_number,
-- bid_due, billing_terms, signer_contact_id, viewer_contact_ids,
-- intro_override. Inserts proposal_recipients for signer + viewers.
...
BEGIN
  ...
  LOOP
    ...
    -- Insert viewers as proposal_recipients
    IF v_target->'viewer_contact_ids' IS NOT NULL
